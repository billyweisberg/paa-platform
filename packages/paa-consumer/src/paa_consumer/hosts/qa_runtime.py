"""QA runtime host bootstrap and loop for the PAA consumer runtime."""
...
from paa_core.config import load_producer_consumer_project_config
...
class _QueueClaimLifecycleAdapter:
    ...
    def requeue_claim(self, claim_id: str) -> dict[str, object]:
        ...
        _, result = client.publish(handoff_runtime.DEFAULT_EXCHANGE, claim['queue'], envelope)
    ...
    def run_qa_verification(self, context: object) -> object:
        ...
        return {
            ...
            'verification_scope': {
                ...
                'scope': 'verify_authorized_slice',
            },
            'mechanical_checks': (
                {'name': 'qa-runtime-host-dry-run', 'status': 'pass'},
            ),
            'technical_scope_checks': (
                {'name': 'assignment-packet-context', 'status': 'pass'},
            ),
            'protected_path_checks': (),
            'artifact_checks': (
                {'name': 'packet-artifact-readable', 'status': 'pass'},
            ),
            'findings': (),
            'recommended_action': {
...
class _QAVerificationPublisher:
        ...
        *,
        ...
        if not isinstance(issue_number, int) or issue_number <= 0:
            return {
                ...
                'details': 'QA verification emission requires a positive issue_number in the source assignment payload.',
            }
        ...
        packet = {
            'message_id': self._build_message_id(issue_number=issue_number, verification_status=str(packet_output.get('verification_status') or 'pass')),
            ...
            'payload': {
                ...
                'pr': {'number': pr_number, 'url': payload.get('pr', {}).get('url') if isinstance(payload.get('pr'), dict) else None, 'ready_for_review': True},
                'verification_status': packet_output.get('verification_status', 'pass'),
                'verification_scope': packet_output.get('verification_scope', {'scope': 'verify_authorized_slice'}),
                'mechanical_checks': list(packet_output.get('mechanical_checks', ())),
                'technical_scope_checks': list(packet_output.get('technical_scope_checks', ())),
                'protected_path_checks': list(packet_output.get('protected_path_checks', ())),
                'artifact_checks': list(packet_output.get('artifact_checks', ())),
                'findings': list(packet_output.get('findings', ())),
                'recommended_action': packet_output.get('recommended_action', {'merge_recommendation': 'accept_and_merge'}),
                'source_slice_packet_ref': {'path': source_packet_path} if source_packet_path else None,
        ...
        output_path = self._verification_output_path(issue_number=issue_number)
    ...
    def _verification_output_path(self, *, issue_number: int) -> Path:
        stamp = datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
        return self._repo_root / '.codex-work' / 'qa-runtime' / f'issue-{issue_number}-qa-verification-{stamp}.json'
    ...
    def _authority_context(self, source_packet: dict[str, Any] | None) -> dict[str, Any]:
        ...
        return {
            ...
            'phase_id': source.get('phase_id') or 'p0-qa-runtime-host',
            'task_id': source.get('task_id') or 'paa-qa-runtime-host',
        }
    ...
    @staticmethod
    def _build_message_id(*, issue_number: int, verification_status: str) -> str:
        slug = verification_status.replace('_', '-')
        stamp = datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
        run_suffix = uuid.uuid4().hex[:8]
        return f'paa-qa-{stamp}-issue{issue_number}-{slug}-{run_suffix}'
...
class QARuntimeLoopResult:
    ...
    emitted_verification: dict[str, Any] | None = None
...
class QARuntimeHost:
    def __init__(
        ...
        *,
        ...
        self._verification_publisher = verification_publisher
        self._actor_name = actor_name
        self._host_name = host_name
        self._logger = logger if logger is not None else _NullStructuredLogger()

    def run_once(self, *, intake_mode: str = 'preview', emit_verification: bool = False) -> QARuntimeLoopResult:
        claim_result = self._claim_with_retry(intake_mode=intake_mode)
        if not claim_result.ok:
            return QARuntimeLoopResult(
        ...
        qa_result = self._qa_worker_service.handle_packet(
            QAWorkerRequest(
        ...
        emitted_verification = None
        if emit_verification and intake_mode == 'claim_next' and self._verification_publisher is not None:
            emitted_verification = self._verification_publisher.publish_verification_result(
                worker_result=qa_result,
        ...
        lifecycle_success = qa_result.ok and (emitted_verification is None or bool(emitted_verification.get('ok')))
        lifecycle_result = self._finalize_claim(
            intake_mode=intake_mode,
            claim_id=(claim_result.claim_summary.claim_id if claim_result.claim_summary else None),
        ...
        *,
        ...
        while True:
            if max_iterations > 0 and count >= max_iterations:
                break
            result = self.run_once(intake_mode=intake_mode, emit_verification=emit_verification)
            result_payload = {
                ...
                'emitted_verification': result.emitted_verification,
...
def build_qa_runtime_host(
    ...
    *,
    ...
    qa_worker_service = DefaultQAWorkerService(
        ...
        methodology_execution_projection_service=methodology_execution_projection_service,
        verification_runner=_DefaultQAVerificationRunner(),
        qa_verification_packet_assembler=_DefaultQAVerificationPacketAssembler(),
        logger=runtime_logger,
    )
    ...
    return QARuntimeHost(
        queue_name=topology.queue_names['qa'],
        ...
        qa_worker_service=qa_worker_service,
        verification_publisher=_QAVerificationPublisher(
