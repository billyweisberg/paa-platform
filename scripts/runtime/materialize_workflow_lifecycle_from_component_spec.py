...
SPEC_PATH = REPO_ROOT / "docs" / "2_Design" / "2026-05-17-workflow-lifecycle-service-component-spec.md"
...
VALUES (
  ...
  {sql_literal('workflow transition coordination from governed component spec')},
  {sql_literal(identity.system_layer)}::paa.system_layer,
  {sql_literal(identity.tier)}::paa.component_tier,
  {sql_literal('Materialized from governed WorkflowLifecycleService component spec tables.')},
...
SELECT implementation_plan_id::text
FROM paa.implementation_plans
WHERE plan_id_external = {sql_literal(seed.plan_seed.plan_name)}
LIMIT 1;
"""
    )
    ...
    realization_ids: dict[tuple[str, str], str] = {
    ...
    plan_repo.upsert_implementation_plan(
        ImplementationPlanUpsertSpec(
            ...
            provenance={
                "source": "scripts/runtime/materialize_workflow_lifecycle_from_component_spec.py",
    ...
    for activity in seed.activity_seeds:
        realization_id = realization_ids[(activity.element_name, activity.realization_kind)]
