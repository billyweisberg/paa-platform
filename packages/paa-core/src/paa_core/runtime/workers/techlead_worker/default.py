...
class DefaultTechLeadWorkerService:
    ...
    def handle_packet(self, request: TechLeadWorkerRequest) -> TechLeadWorkerResult:
        ...
        if packet_schema_type == 'qa_verification_packet':
            result = self._handle_qa_verification_packet(
                request=request,
        ...
        self._logger.info(
            ...
            methodology_execution_id=methodology_execution_id,
            ok=result.ok,
            recommended_next_action=result.dispatch_summary.recommended_next_action,
        )
        return result

    def _handle_qa_verification_packet(
        self,
        *,
        ...
        acceptance_request = TechLeadAcceptanceDecisionRequest(
            project_slug=str(payload.get('project_slug') or ''),
            issue_number=issue_number,
            pr_number=pr_number,
            workflow_stage=self._normalize_qa_review_stage(str(payload.get('workflow_stage') or '')),
            qa_result_type=str(payload.get('verification_status') or ''),
            source_packet_schema_type=request.packet_schema_type,
            source_packet_message_id=request.packet_message_id,
            metadata=request.metadata,
        )
        acceptance_result = self.techlead_acceptance_decision_service.derive_acceptance_decision(
            acceptance_request
        )
        dispatch_summary = TechLeadWorkerDispatchSummary(
            ...
            decision_service_used='TechLeadAcceptanceDecisionService',
            decision_supported=acceptance_result.summary.decision_supported,
            recommended_next_action=acceptance_result.summary.recommended_next_decision,
            recommended_target_role='TechLead' if acceptance_result.summary.acceptance_allowed else None,
            packet_emission_required=False,
            methodology_transition_required=acceptance_result.summary.closeout_allowed,
            blocking_reasons=acceptance_result.summary.blocking_reasons,
            notes=acceptance_result.summary.notes,
        )
        normalized_packet_output_summary = (
            acceptance_result.summary.decision_summary if acceptance_result.ok else None
        )
        return TechLeadWorkerResult(
            ...
            details=acceptance_result.details,
            dry_run=True,
            metadata={
                'service_component': 'TechLeadWorkerService',
    ...
    @staticmethod
    def _normalize_qa_review_stage(workflow_stage: str) -> str:
        normalized = workflow_stage.strip()
        if normalized == 'qa_verification_complete':
