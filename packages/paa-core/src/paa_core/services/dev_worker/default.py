"""Default implementation for the Dev worker service."""
...
from .contracts import DevExecutionRunner, WorkerResultPacketAssembler
from .models import DevWorkerExecutionSummary, DevWorkerRequest, DevWorkerResult
...
class DefaultDevWorkerService:
    """Coordinate the first supported deterministic Dev worker-host slice."""
    ...
    def __init__(
        ...
        *,
        ...
        execution_runner: DevExecutionRunner,
        worker_result_packet_assembler: WorkerResultPacketAssembler,
        ...
        self._worker_result_packet_assembler = worker_result_packet_assembler
    ...
    def worker_result_packet_assembler(self) -> WorkerResultPacketAssembler:
        return self._worker_result_packet_assembler
    ...
    def handle_packet(self, request: DevWorkerRequest) -> DevWorkerResult:
        ...
        self._logger.info(
            'dev_worker.handle_packet.start',
        ...
        if request.runtime_mode != 'dry_run':
            return self._build_blocked_result(
                ...
                details='Live Dev worker execution is not supported in the first service slice.',
        ...
        execution_result = self.execution_runner.run_dev_execution(packet_context_result)
        packet_output = self.worker_result_packet_assembler.assemble_worker_result_packet(execution_result)
        execution_summary = DevWorkerExecutionSummary(
            handler_key='dev-assignment-dry-run',
            packet_schema_type=packet_schema_type,
            runtime_mode=request.runtime_mode,
            execution_supported=True,
            execution_runner_used=type(self.execution_runner).__name__,
            ...
            blocking_reasons=(),
            notes=('dry-run-only', 'result-packet-normalized'),
        )
    ...
    def _build_context_blocked_result(
        ...
        self._logger.warning(
            'dev_worker.handle_packet.context_blocked',
    ...
    def _build_blocked_result(
        ...
        *,
        ...
        self._logger.warning(
            'dev_worker.handle_packet.blocked',
            packet_schema_type=request.packet_schema_type,
            reason=reason,
        )
        return DevWorkerResult(
    ...
    def _normalize_packet_output_summary(self, packet_output: object) -> str:
        ...
        return f'Dry run only: worker result output prepared as {type(packet_output).__name__}.'


__all__ = ['DefaultDevWorkerService']
