...
| --- | --- | --- | --- | --- | --- |
| Authority manifest resolution and issue/task materialization | Yes | `/Users/billyweisberg/.codex/skills/fractal-core-authority/scripts/project_authority.py` and duplicate copies under `/Users/billyweisberg/Repos/Individual-Centricity/appdev/tools/codex-skills/fractal-core-authority/scripts/project_authority.py` and `/Users/billyweisberg/Repos/Individual-Centricity/appdev-authority-source/tools/codex-skills/fractal-core-authority/scripts/project_authority.py` | Resolve active task, sync issue, compile packets, write PAA rows | `packages/paa-producer/src/paa_producer/authority_runtime.py` plus shared DB/path helpers in `packages/paa-core/src/paa_core/` | Remove hardcoded `~/.codex/authority/...`; remove fallback to `appdev-arch`; replace direct repo path guesses with producer config and installed authority package paths |
| Authority publication | Yes | `/Users/billyweisberg/.codex/skills/fractal-core-authority/scripts/publish_current.py`, `/Users/billyweisberg/Repos/Individual-Centricity/appdev/tools/codex-skills/fractal-core-authority/scripts/publish_current.py` | Publish current authority package / mirrors | Already partially moved to `packages/paa-producer/src/paa_producer/publish.py`; continue by extracting remaining wrapper behavior | Remove repo-specific mirror fanout assumptions; replace with producer config-driven package publish and optional consumer install step |
| Coder-brief readiness materializer | Yes | `/Users/billyweisberg/.codex/skills/fractal-core-authority/scripts/materialize_coder_brief_readiness.py`; duplicate in `/Users/billyweisberg/Repos/Individual-Centricity/appdev-authority-source/tools/codex-skills/fractal-core-authority/scripts/materialize_coder_brief_readiness.py` | Compute and optionally persist readiness state and dependency status for coder briefs | `packages/paa-core/src/paa_core/readiness.py` for algorithm and DB access; producer/consumer entrypoints in `paa-producer` and `paa-consumer` | Remove direct docker/psql shell coupling from business logic; remove assumption that source briefs must come from ad hoc directories only; support producer artifacts and installed consumer packages via config |
| RabbitMQ handoff runtime and DB-backed queue trace persistence | Yes | `/Users/billyweisberg/.codex/skills/fractal-core-handoff-common/scripts/rabbitmq_handoff.py`; duplicate in `appdev` and `appdev-authority-source` skill copies | Queue check/claim/send/ack/requeue; persist handoff linkage into `paa.automation_runs`, `paa.handoffs`, `paa.queue_messages` | `packages/paa-core/src/paa_core/handoff_runtime.py` plus CLI surfaces in `paa-consumer` and shared runtime templates | Remove hardcoded RabbitMQ defaults as the only mode; replace queue names and state roots with consumer config; remove dependence on `$HOME/.codex/state` as primary claim root |
| TechLead report generation and DB persistence | Yes | `/Users/billyweisberg/.codex/skills/fractal-core-techlead/scripts/techlead_status.py` | Reconcile authority, queue state, GitHub state, automation visibility, traceability; persist report into `paa.automation_runs` | `packages/paa-consumer/src/paa_consumer/techlead.py` with shared helpers in `paa-core`; schema under `schemas/runtime-records/` | Remove hardcoded `appdev-arch`; remove hardcoded paths to `~/.codex/authority`, `~/.codex/automations`, QA work dirs, and role workspace roots; drive entirely from consumer repo config and installed runtime |
| Inbox / claim workflow skill layer | Yes, but thin | Skill wrappers under `/Users/billyweisberg/.codex/skills/fractal-core-inbox/`, `/Users/billyweisberg/.codex/skills/fractal-core-queue-admin/`, `/Users/billyweisberg/.codex/skills/fractal-core-dev-result/`, `/Users/billyweisberg/.codex/skills/fractal-core-qa-review/`, `/Users/billyweisberg/.codex/skills/fractal-core-architect-handoff/` | Role-facing entrypoints over the shared queue and authority runtime | Repo-local templates under `templates/skills/` and `templates/automations/`; generated/installable payloads from `paa-platform` | Remove primary dependence on home-installed skills; treat these as install artifacts produced from `paa-platform` into repo-local `.codex/` |
| Packet schemas and reference examples | Yes | `/Users/billyweisberg/.codex/skills/fractal-core-handoff-common/references/` and duplicates under `appdev/tools/codex-skills/fractal-core-handoff/fractal-core-handoff-common/references/` | Validate and document `architect_cycle_packet`, `slice_result_packet`, `qa_verification_packet` | `schemas/handoff-packets/` and `templates/packet-examples/` inside `paa-platform` | Remove duplicated schema copies across skills and producer repos; keep one canonical platform source |
| Full-chain reporting view and proof queries | Yes | SQL docs under `/Users/billyweisberg/Repos/Individual-Centricity/appdev-authority-source/docs/architecture/tom-baby7-fractal-core/100-paa-single-slice-full-chain-proof-queries.sql`, `/Users/billyweisberg/Repos/Individual-Centricity/appdev-authority-source/docs/architecture/tom-baby7-fractal-core/102-paa-full-chain-reporting-view.sql`, `/Users/billyweisberg/Repos/Individual-Centricity/appdev-authority-source/docs/architecture/tom-baby7-fractal-core/98-paa-full-chain-proof-queries.sql` | DB reporting and proof queries for traceability | `migrations/` and `packages/paa-core/src/paa_core/sql/` in `paa-platform` | Remove assumption that reporting SQL lives only as architecture docs in transitional repos; promote to versioned platform-owned SQL assets |
...
- `/Users/billyweisberg/.codex/skills/fractal-core-authority/scripts/project_authority.py`
- `/Users/billyweisberg/.codex/skills/fractal-core-authority/scripts/materialize_coder_brief_readiness.py`
- `/Users/billyweisberg/.codex/skills/fractal-core-handoff-common/scripts/rabbitmq_handoff.py`
- `/Users/billyweisberg/.codex/skills/fractal-core-techlead/scripts/techlead_status.py`
...
- `packages/paa-producer/src/paa_producer/cli_*.py` as the CLI grows
...
Current assumptions include direct references to:
- `/Users/billyweisberg/Repos/billyweisberg/fractal-core-python-automation-dev-source`
- `/Users/billyweisberg/Repos/billyweisberg/fractal-core-python-automation-qa-source`
