...
def persist_packet_compilation(
    *,
    project_slug: str,
    packet: dict,
    package_id_external: Optional[str],
    brief_id_external: Optional[str],
    review_markdown: Optional[str],
    output_path: Optional[str],
    review_output_path: Optional[str],
    source_input_path: Optional[str] = None,
    source_packet_path: Optional[str] = None,
) -> str:
...
def derive_keep_stable(package: dict):
...
def derive_focus(selected_brief: dict, package: dict):
    focus = []
...
def derive_next_move(selected_brief: dict, next_issue_number: int):
...
def derive_remaining_gap(task: Optional[dict], package: dict):
    if task and task.get('authoring', {}).get('current_gap'):
        current_gap = task['authoring']['current_gap']
        if isinstance(current_gap, list):
            return ' '.join(current_gap[:2])
        return str(current_gap)
    gaps = package.get('implementation_target', {}).get('current_gap', [])
    if gaps:
        return ' '.join(gaps[:2])
    return package.get('product_and_source_basis', {}).get('product_outcome_statement', 'remaining implementation gap requires the next execution-ready slice')


def derive_governance_reminders():
    return list(DEFAULT_GOVERNANCE_REMINDERS)


def write_review_markdown(path: Path, packet: dict):
...
def build_parser():
    ...
    p.add_argument('--packet-project', default='fractal-core')
    p.add_argument('--to-role', default='qa')
    p.add_argument('--repo', required=True)
    p.add_argument('--issue-number', type=int, required=True)
    ...
    p.add_argument('--brief-id-external', required=True)
    p.add_argument('--packet-project', default='fractal-core')
    p.add_argument('--to-role', default='architect')
    p.add_argument('--repo', required=True)
    p.add_argument('--issue-number', type=int, required=True)
    p.add_argument('--issue-url', required=True)
    p.add_argument('--pr-number', type=int, required=True)
